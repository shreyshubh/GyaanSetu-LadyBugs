  ...
  return (
    <div style={{ display: 'flex', gap: '24px', height: 'calc(100vh - 64px)' }}>
      ...
      <div className="card" style={{ flex: '0 0 30%', overflowY: 'auto' }}>
        ...
        {/* Topic Context info moved to chat header */}
      </div>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '24px', display: 'flex', flexDirection: 'column', gap: '24px' }}>
          {messages.map((msg, idx) => (
            <div key={idx} style={{ display: 'flex', flexDirection: 'column', alignItems: msg.role === 'user' ? 'flex-end' : 'flex-start' }}>
              ...
              <div style={{ background: msg.role === 'user' ? 'var(--accent-light)' : 'var(--surface)', border: msg.role === 'assistant' ? '1px solid var(--border)' : 'none', padding: '12px 16px', borderRadius: '6px', maxWidth: '80%', whiteSpace: 'pre-wrap' }}>{msg.content}</div>
            </div>
          ))}
